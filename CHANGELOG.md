...
- **MRI schema v2 — slug-keyed orphans + inline citation rendering** — every `MRI.refs[]` entry now carries `resolvedDocId`, `needsResolve` (`null` / `"known-publisher-no-doc"` / `"unknown-publisher"`), and a `contentHash` for cross-sighting dedup. Unparseable refs (the former `badRefs` cohort + the `orphans.unmapped[]` flat list) are lifted into `refs[]` keyed by source-anchored slugs (`orphan/<sourceDoc>/<refXmlId>`). Build-side: `getStatus` returns `"MRI-KNOWN"` for refs MRI knows about but the registry doesn't carry as docs — `docId.hbs` renders them inline as `<cite>refId — citation text</cite>` with an `EXTERNAL` badge instead of the bare `NOT IN REGISTRY` pill. Silences 1,298 `No lineage key derivable` + 120 `not found in registry` build warnings where MRI has the citation info. Client-side: new `build/api/mri-cite-map.json` sidecar (~150 KB) lets `refTree.js` show docId + cite + `EXTERNAL` on tree nodes whose target isn't in the registry. One-time `migrateMriToSlugSchema.js` extras script handles the in-place schema lift; `referencing.js` mints the slug schema going forward.
...
- **Validator catches case-only git/FS drift** — `src/main/scripts/validate.js` now cross-checks `git ls-files` against `fs.readdirSync` per doc and against the in-file `docId`. Catches the macOS-case-insensitive-FS class of bug that bit #1188/#1189 (filename uppercase on disk + lowercase docId in content) locally before push, instead of waiting for CI on Linux.
